...
VERSION = "7.2.3-v22-shared-symbol-leverage-guard"
...
class AccountManager:
    ...
    def prepare_leverage_for_open(self, symbol: str, requested: int) -> Optional[int]:
        ...
        position.  More importantly, increasing leverage while another strategy is open
        changes that existing position's liquidation risk.  Therefore leverage is changed
        only while the symbol is flat.  With an open position we may reuse an equal or
        LOWER leverage; a higher existing leverage than requested is blocked until flat.
        """
        requested = max(MIN_LEVERAGE, min(int(requested), MAX_REQUESTED_LEVERAGE,
                                          BOT_HARD_MAX_LEVERAGE, API_HARD_MAX_LEVERAGE))
        if PYRAMID_ENGINE_ENABLED:
            requested = min(requested, max(MIN_LEVERAGE, PYRAMID_LEVERAGE))
        if not LIVE_TRADING:
        ...
        if current > requested:
            logger.warning(
                f"LEVERAGE ENTRY BLOCK V22 | {symbol} | current={current}x requested={requested}x "
                f"posicoes_abertas={open_count} | ISOLATED nao permite reduzir com posicao aberta; "
                f"aguardando simbolo ficar FLAT"
            )
            return None
        if current < requested:
            # Lower leverage is safer for liquidation.  Reuse it and never raise leverage
            # under another strategy's live position.  ExecutionEngine revalidates margin.
            logger.info(
                f"LEVERAGE REUSE V22 | {symbol} | current={current}x requested={requested}x "
                f"posicoes_abertas={open_count} | mantendo leverage menor"
            )
            return current
        logger.info(f"LEVERAGE REUSE V22 | {symbol} | {current}x | posicoes_abertas={open_count}")
        return current
...
class ExecutionEngine:
    ...
    def open_leg(self, strategy_id: str, symbol: str, position_side: str, sizing: Dict[str, Any],
        ...
        if effective_leverage is None:
            logger.warning(
                f"OPEN BLOCK LEVERAGE V22 | {strategy_id} | {symbol} {position_side} | "
        ...
        if effective_margin > free_margin:
            logger.warning(
                f"OPEN BLOCK MARGIN V22 | {strategy_id} | {symbol} | notional={actual_notional} "
